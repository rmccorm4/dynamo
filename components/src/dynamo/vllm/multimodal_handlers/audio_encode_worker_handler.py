...
class AudioEncodeWorkerHandler:
    ...
    1. Parses the audio_url from the incoming Rust SDK request (ModelInput.Tokens
       format: ``{"token_ids": [...], "multi_modal_data": {"audio_url": [...]}}``)
    2. Downloads and encodes the audio using the model's audio tower +
       multi_modal_projector via ``get_audio_embeddings()``.
    ...
    4. Forwards a ``vLLMMultimodalRequest`` (with ``embeddings_shape`` and
       ``serialized_request`` populated, ``audio_url`` cleared) to the PD
       worker and streams responses back to the caller.
    """
    ...
    def _parse_frontend_request(self, raw_request: dict) -> tuple[vLLMMultimodalRequest, str | None]:
        """Parse a raw Rust SDK request dict into a vLLMMultimodalRequest and audio URL.

        The Rust SDK sends ``{"token_ids": [...], "multi_modal_data":
        {"audio_url": [{"Url": "https://..."}]}}`` when registered as
        ModelInput.Tokens.
        """
    ...
    async def generate(self, raw_request: dict, context) -> AsyncIterator[str]:
        ...
        2. Download audio via AudioLoader.
        3. Pre-process with AutoProcessor.
        4. Run through audio tower + projector.
        5. Transfer embeddings to PD worker via RDMA (create_readable pattern).
        6. Forward vLLMMultimodalRequest with embeddings metadata.
        ...
        try:
            ...
            with await self._connector.create_readable(descriptor) as readable:
                # Populate embeddings metadata; clear audio_url to signal that
                # audio is passed as embeddings, not as a URL.
                request.embeddings_shape = list(audio_embeddings.shape)
                request.serialized_request = readable.metadata()

                # multimodal_inputs is a list of MultiModalGroup; for audio we
                # store the transfer metadata at the top-level request fields
                # (embeddings_shape / serialized_request) rather than in the
                # per-item list, matching the example encoder pattern.
            ...
            logger.error("Error processing audio encode request %s: %s", request_id, e)
            raise
